...
import {Raza} from "./../controller/Raza.js";
import {parametros} from "./../controller/parametros.js"
import { jwtVerify } from "jose";

const proxyRaza = express();
proxyRaza.use(async(req, res, next)=>{
    try{
        ...
        if (req.method === 'POST' || req.method === 'PUT'){
            let data = plainToClass(Raza, jwtData.payload.body, {excludeExtraneousValues: true});
            await validate(data);
        }
        next();
    } catch(err) {
