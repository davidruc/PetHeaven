...
import {Afiliacion} from "./../controller/Afiliacion.js";
import {parametros} from "./../controller/parametros.js"
import { jwtVerify } from "jose";

const proxyAfiliacion = express();
proxyAfiliacion.use(async(req, res, next)=>{
    try{
        ...
        if (req.method === 'POST' || req.method === 'PUT'){
            let data = plainToClass(Afiliacion, jwtData.payload.body, {excludeExtraneousValues: true});
            await validate(data);
        }
        next();
    } catch(err) {
...
export default proxyAfiliacion;