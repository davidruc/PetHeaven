...
import {Seguimiento} from "./../controller/Seguimiento.js";
import {parametros} from "./../controller/parametros.js"
import { jwtVerify } from "jose";

const proxySeguimiento = express();
proxySeguimiento.use(async(req, res, next)=>{
    try{
        ...
        if (req.method === 'POST' || req.method === 'PUT'){
            let data = plainToClass(Seguimiento, jwtData.payload.body, {excludeExtraneousValues: true});
            await validate(data);
        }
        next();
    } catch(err) {
...
export default proxySeguimiento;