...
import {EstadoPlan} from "./../controller/EstadoPlan.js";
import {parametros} from "./../controller/parametros.js"
import { jwtVerify } from "jose";

const proxyEstadoPlan = express();
proxyEstadoPlan.use(async(req, res, next)=>{
    try{
        ...
        if (req.method === 'POST' || req.method === 'PUT'){
            let data = plainToClass(EstadoPlan, jwtData.payload.body, {excludeExtraneousValues: true}); 
            await validate(data);
        }
        next();
    } catch(err) {
...
export default proxyEstadoPlan;