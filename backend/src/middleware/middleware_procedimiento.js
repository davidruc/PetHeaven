...
import {Procedimiento} from "./../controller/Procedimiento.js";
import {parametros} from "./../controller/parametros.js"
import { jwtVerify } from "jose";

const proxyProcedimiento = express();
proxyProcedimiento.use(async(req, res, next)=>{
    try{
        ...
        if (req.method === 'POST' || req.method === 'PUT'){
            let data = plainToClass(Procedimiento, jwtData.payload.body, {excludeExtraneousValues: true}); 
            await validate(data);
        }
        next();
    } catch(err) {
...
export default proxyProcedimiento;